...
public class ProductDAOImpl implements ProductDAOExt {
	...
	public ProductD findProductByProductStepUpId(int productStepUpId) {
		...
		Query query = em.createNativeQuery(
				"SELECT P.I_PRODUCT,P.PRODUCT_CODE,P.PRODUCT_NAME,PUT.UNIT_TYPE_NAME,IFNULL(QUANTITY-SOLD_QUANTITY,0) AS STOCK_LEVEL,ROUND(PAYMENT_AMOUNT/QUANTITY,3) as COST,ROUND(PAYMENT_AMOUNT/QUANTITY,3) as PRICE,SCIENTIFIC_NAME,COUNTRY,PACKET_SIZE,I_PRODUCT_STEPUP\n"
		...
		productD.setProductStepUpId(Integer.parseInt("" + row[10]));
